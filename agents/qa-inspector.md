...
- Run `node tools/geometry-audit.mjs <served url> --fold-selector "<primary CTA>" --assert "<key selector>:font-weight:<expected>" --assert "<primary CTA>:background-color:<expected rgb>"` (serve first; requires system Chrome) — rendered pixels at 375x667/560/640/768/1280: overflow, form-row alignment, 44px touch targets, button-height consistency, image squish, real CPL (>80 fails), the CTA fold contract, and computed-style regression asserts. Source lint passing does not imply this passes.
